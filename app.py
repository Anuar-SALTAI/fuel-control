...
.fc-sidebar-card{width:90%;max-width:185px;margin:1.5rem auto .75rem;padding:0;background:transparent;border:0;box-shadow:none}
.fc-sidebar-photo{display:block;width:100%;height:270px;object-fit:cover;object-position:center 35%;border-radius:14px;box-shadow:0 5px 16px rgba(15,45,92,.1)}.fc-sidebar-brand{text-align:center;color:#173557;padding:10px 2px 0;font-size:.9rem;line-height:1.35}.fc-sidebar-brand b{font-weight:650}.fc-sidebar-brand span{font-weight:400;color:#5a718d}
@media(max-height:760px){.fc-sidebar-card{margin-top:1rem}.fc-sidebar-photo{height:190px}}
...
.fc-sidebar-card{width:88%;max-width:185px}.fc-sidebar-photo{height:240px}
.fc-sidebar-brand{font-size:.88rem}
...
@media(max-width:700px) and (max-height:760px){.fc-sidebar-photo{height:185px}}
</style>""",
...
st.markdown(
    ...
    f'</div><a class="fc-profile" href="?lang={language}&profile=1" target="_self"><svg aria-label="Profile" viewBox="0 0 24 24" width="23" height="23" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="9" r="3"/><path d="M6.5 19c.8-3 2.7-4.5 5.5-4.5s4.7 1.5 5.5 4.5"/></svg></a></div></div>',
    unsafe_allow_html=True,
)

force_local = os.getenv("FUEL_CONTROL_LOCAL", "").lower() in {"1", "true", "yes"}
require_supabase = os.getenv("FUEL_CONTROL_BACKEND", "").lower() == "supabase"
try:
    supabase = (
        None
        if force_local
        else create_supabase_client(st.secrets, required=require_supabase)
    )
except SupabaseConfigurationError:
    st.error(t["supabase_missing"])
    st.stop()
local_mode = supabase is None
auth = None
if supabase is not None:
    auth = AuthManager(supabase, st.session_state)
    if st.session_state.get("access_token") and st.session_state.get("refresh_token"):
        auth.restore()

if not local_mode and not auth.authenticated:
    ...
    with login_tab:
        with st.form("login", clear_on_submit=True):
            ...
            if st.form_submit_button(
                t["login"], type="primary", use_container_width=True
            ):
                try:
    ...
    with register_tab:
        with st.form("register", clear_on_submit=True):
            ...
            if st.form_submit_button(t["register"], use_container_width=True):
                if len(password) < 8:
                    st.error(t["password_length"])
                else:
                    try:
                        signed_in = auth.sign_up(name, email, password)
                        if signed_in:
                            st.rerun()
                        st.success(t["registration_confirmation"])
                    except Exception:
                        st.error(t["registration_error"])
    st.stop()

if local_mode:
    db = Database()
else:
    db = SupabaseRepository(supabase, st.session_state.user_id, st.session_state.role)
    db.ensure_default_vehicle()
    if st.query_params.get("profile") == "1":
        with st.container(border=True):
            st.write(f"**{st.session_state.display_name}**")
            st.caption(st.session_state.email)
            st.caption(f'{t["role"]}: {st.session_state.role}')
...
if page_key == "new":
    ...
    if not local_mode and st.session_state.role == "admin":
        st.subheader(t["users"])
        profiles = db.profiles()
        st.dataframe(profiles, use_container_width=True, hide_index=True)
        profile_ids = {
            f'{item["full_name"]} · {item["email"]}': item for item in profiles
        }
        selected_profile = st.selectbox(t["user"], list(profile_ids))
        selected_user = profile_ids[selected_profile]
        active = st.checkbox(t["account_active"], value=selected_user["is_active"])
        if st.button(t["save_user"]):
            db.set_profile_active(selected_user["id"], active)
